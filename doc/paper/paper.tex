...
We here assume that all instances $P$ of the generalized problem have a set of actions $A$. We do not need $A$ begin fixed.
...
\begin{itemize}
 ...
 \item $Good(s, a, s')$ for $s$ alive, $s'$ solvable, and $(s, a, s') \not\in \badtx$.
...
\smallpar{C5-6}
Good transitions $(s, a, s')$ can be distinguished from bad transitions $(s, a, s')$.
...
(which implies that $s'$ is solvable). Where $a, b \in A$. Then,
\begin{align}
 Good(s, a, s') \rightarrow Good(t, b, t') \lor
...
The transitions $(s, a, s')$ and $(s, a, t')$ will be treated as equivalent. In $C5-6$ if $a=b$ and $s=t$ we don't require this transitions to be distinguished.

\smallpar{C8}
If $(s, a)$ is good, then the resulting transitions must be good too:
%(s, a, s') \in T \setminus \badtx
\begin{align}
Good(s, a) \rightarrow Good(s, a, s'),\;\; \text{for $(s, a, s') \in T$ where $s$ alive.}
\end{align}

\smallpar{C9 (Optional)}
...
\smallpar{C12 (Optional)}
If no action is good in state $s$ alternatively we can set the state as $Bad(s)$. Then  post a constraint $\neg Bad(s)$, with weight equal to one. Then the constraint $C1$ changes,
\begin{align}
Bad(s) ~ or~ \bigvee_{a \in A} Good(s, a),&\;\; \text{for $s$ alive.}
\end{align}

\noindent Notice that now the policy is complete in domains where an state can not be solved or has not any $Good(s, a)$.
