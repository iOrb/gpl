...
\subsection{Fixed Pool $A$ of actions}

We here assume that all instances $P$ of the generalized problem have the same fixed set of actions $A$.
We denote by $(s, a, s'') \in T$ tuples of state, action, and possible state $s''$ resulting from the adversary
response to the application of $a$ in $s$.
...
\begin{align}
 Good(s, a) \land V(s'', d) \rightarrow \bigvee_{d < k \leq D} V(s, k),&\;\; \text{for $s$ alive, $(s, a, s'') \in T$, $d \in [1, D)$.} \\
 V(s'', D) \rightarrow \neg Good(s, a),&\;\; \text{for $s$ alive, $(s, a, s'') \in T$.} \tag{\theequation${}^\prime$}
\end{align}
