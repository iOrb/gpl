        ...
        CNFWriter& wr, const std::vector<transition_pair>& transitions_to_distinguish)
{
    ...
    for (unsigned tx=0; tx < num_alive_transitions; ++tx) {
        ...
        if (tx == repr) { // tx is an equivalence class representative: create the Good(s, s') variable
            cnfvar_t good_s_sprime = wr.var("Good(" + std::to_string(s) + ", " + std::to_string(sprime) + ")");
    ...
    for (const auto& [s, a, sprime]:tr_set_.transitions().nondet_transitions()) {
        if (is_alive(s)) {
    ...
    for (const auto s:all_alive()) {
        ...
        for (const auto a:s_to_as[s]) {
            ...
            for (const auto& sprime:s_a_to_s[{s, a}]) {
                ...
                wr.cl({Wr::lit(good_s_a.at({s, a}), false),
                       Wr::lit(goods.at(get_representative_id(tx)), true)});
                n_good_sa_clauses += 1;
            }
        }
    ...
    std::cout << "\tGood(s, a): " << good_s_a.size() << std::endl;
    std::cout << "\tV(s, d): " << n_v_vars << std::endl;

    // Check our variable count is correct
    assert(wr.nvars() == n_select_vars + goods.size() + n_v_vars + good_s_a.size());
    ...
    std::cout << "\tNon-det (Good(s, a)) completeness [1]: " << n_good_sa_clauses << std::endl;
    ...
    assert(wr.nclauses() == n_selected_clauses + n_good_tx_clauses + n_good_sa_clauses + n_descending_clauses
