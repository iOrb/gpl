    ...
    for (const auto& s:sample_.expanded_states()) {
        for (const auto& a:sample_.s_as(s)) {
            for (const auto& sp:sample_.successors({s, a})) {
                auto tx = std::make_tuple((state_id_t) s, (action_id_t) a, (state_id_t) sp);
...
void D2LEncoding::report_eq_classes() const {
    std::unordered_map<unsigned, std::vector<tx_triple>> classes;
    for (unsigned tx=0; tx < transition_ids_.size(); ++tx) {
        auto repr = get_representative_id(tx);
        classes[repr].push_back(get_tx_triple(tx));
    }
    ...
    for (const auto& elem:classes) {
        ...
        for (unsigned j = 0; j < elements.size(); ++j) {
            ...
            std::cout << "(" << get_s(tx) << ", " << get_a(tx) << ", " << get_sp(tx) << ")";
...
//                std::cout << s << ", " << d << std::endl;
    ...
    // Create a variable "Good(s, a, s')" for each transition (s, a, s') such that s' is solvable and (s, a, s') is not in BAD
    for (unsigned tx=0; tx < num_transitions; ++tx) {
        if (is_necessarily_bad(tx)) continue; // This includes  alive-to-dead transitions

        const auto& tx_ = get_tx_triple(tx);
        const auto s = get_s(tx_), a = get_a(tx_), sp = get_sp(tx_);

        cnfvar_t good_s_a_sp = 0;
        auto repr = get_representative_id(tx);
        if (tx == repr) { // tx is an equivalence class representative: create the Good(s, a, s') variable
            good_s_a_sp = wr.var("Good(" + std::to_string(s) + ", " + std::to_string(a) + ", " + std::to_string(sp) + ")");
            auto it = variables.goods.emplace(tx, good_s_a_sp);
            assert(it.second); // i.e. the SAT variable Good(s, a, s') is necessarily new
            varmapstream << good_s_a_sp << " " << s << " " << a << " " << sp << std::endl;

        } else {  // tx is represented by repr, no need to create a redundant variable
            good_s_a_sp = variables.goods.at(repr);
        }
    }
    ...
    if (options.verbosity>0) {
        ...
        std::cout << "\tGood(s, a, s'): " << variables.goods.size() << std::endl;
    ...
    /////// CNF constraints ///////
    // [1] For each alive state s, post a constraint OR_{s' solvable child of s} Good(s, a, s')
    //for (const auto s:sample_.alive_states()) {
    for (const auto s:sample_.expanded_states()) {
        cnfclause_t clause;
        for (const auto a:sample_.s_as(s)) {
            for (unsigned sp:sample_.successors({s, a})) {
                auto tx = get_transition_id(s, a, sp);
