...
class TransitionSample:
    ...
    def add_transitions(self, states, transitions, instance_id, deadends):
        ...
        mdp_parents = compute_mdp_parents(transitions)
        self.mdp_parents.update(mdp_parents)
        self.parents.update(compute_parents_from_mdp_parents(mdp_parents))
        for s in states:
...
def log_sampled_states(sample, filename):
    ...
    transitions = compute_parents_from_mdp_transitions(sample.transitions)
...
def compute_mdp_parents(transitions):
    mdp_parents = defaultdict(set)
    for source, targets in transitions.items():
        for t in targets:
            mdp_parents[t].add(source)
    return mdp_parents


def compute_parents_from_mdp_parents(mdp_parents):
    parents = defaultdict(set)
    for keys, values in mdp_parents.items():
        for k in keys:
            parents[k].update(values)
    return parents


def compute_parents_from_mdp_transitions(mdp_transitions):
    transitions = defaultdict(set)
    for key, values in mdp_transitions.items():
        for v in values:
            transitions[key].update(v)
    return transitions
...
def remap_state_ids(states, goals, transitions, deadends, remap):
    ...
    for source, targets in transitions.items():
        s = remap(source)
        new_transitions[s] = set()
        for ts in targets:
            new_transitions[s].add(frozenset({remap(t) for t in ts}))
        # new_transitions[remap(source)] = {remap(t) for ts in targets for t in ts}
...
def read_single_sample_file(filename):
    ...
    for line in read_file(filename):
        if line.startswith('(E)'):  # An edge, with format "(E) 5 12 6 ..."
            ids = [int(x) for x in line[4:].split(' ')]
            transitions[ids[0]].add(frozenset(ids[1:]))
            nlines += 1
...
def print_transition_matrix(sample, transitions_filename):
    ...
    with open(transitions_filename, 'w') as f:
        ...
        for s in state_ids:
            pairs = []
            for i, succs in enumerate(transitions.get(s, []), start=0):
                pairs += [(i, sprime) for sprime in sorted(succs)]
